...
/// A list of RDF terms
pub type TermList<'a, F> = Vec<MownTerm<'a, F>>;

/// A predicate with a list of objects
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PoList<'a, F>
where
    F: Format + Valid<MownStr<'a>>,
{
    predicate: MownTerm<'a, F>,
    ...
    F: Format + Valid<MownStr<'a>>,
{
    ...
    pub fn new(predicate: MownTerm<'a, F>, objects: TermList<'a, F>) -> Self {
    ...
    pub fn into_iter(self) -> impl Iterator<Item = (MownTerm<'a, F>, MownTerm<'a, F>)> {
        let p = self.predicate;
        self.objects.into_iter().map(move |o| (p.clone(), o))
    }
    /// Returns the list of predicates and objects by reference.
    pub fn iter(&self) -> impl Iterator<Item = (&MownTerm<'a, F>, &MownTerm<'a, F>)> {
...
{
    /// Creates a new PO-list from a predicate and a list of objects.
    pub fn new(subject: MownTerm<'a, F>, po_lists: Vec<PoList<'a, F>>) -> Self {
    ...
    /// copied!
    #[allow(clippy::should_implement_trait)] // TODO: Implement IntoIterator
    pub fn into_iter(self) -> impl Iterator<Item = [MownTerm<'a, F>; 3]> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = [&MownTerm<'a, F>; 3]> {
        self.po_lists
            .iter()
